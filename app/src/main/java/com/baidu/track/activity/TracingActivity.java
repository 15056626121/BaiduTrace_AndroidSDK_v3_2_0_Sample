...
public class TracingActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_trace:
                boolean isAccessPermission = SharedPreferenceUtil.getBoolean(TracingActivity.this,
                        Constants.PERMISSIONS_DESC_KEY, false);
                if (!isAccessPermission) {
                    Toast.makeText(TracingActivity.this, "需要同意隐私条款后才可以采集", Toast.LENGTH_LONG).show();
                    return;
                }
                if (trackApp.mClient == null) {
                    Toast.makeText(TracingActivity.this,
